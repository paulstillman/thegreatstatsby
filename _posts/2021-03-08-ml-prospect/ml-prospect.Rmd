...
author:
  - name: Paul E. Stillman
    url: https://paulstillman.com/blog
date: 03-08-2021
...
This is a brief introduction to how to use maximum likelihood to estimate the prospect theory parameters of loss aversion ($\lambda$) and diminishing marginal utility ($\rho$) using the `optim` function in R. The first part is meant to go through the logic and math behind prospect theory and modeling choices. The second part goes in detail into the R code used to do the Maximum likelihood estimation plus a few of the pitfalls that you can run into. If you're already familiar with MLE and just want the code, you can skip to the last section, which has the full code. All data and code for this page can be found [here](https://github.com/paulstillman/prospect-estimation) (If you're following along, you'll want to download "data_all_2021-01-08.csv").

For the R portions, it will be helpful to be familiar with the [pipe](https://r4ds.had.co.nz/pipes.html), [dplyr](https://dplyr.tidyverse.org/), [purrr](https://r4ds.had.co.nz/iteration.html#the-map-functions), and [nested data/broom](https://cran.r-project.org/web/packages/broom/vignettes/broom_and_dplyr.html).
...
Because this can take a while, I'm going to use the `furrr` package, which allows for parallel computing in conjunction with `purrr` (i.e., the package that contains the `map` functions). If you don't want to mess around with this, you can delete the first two lines of code here, and replace `future_map` with just `map` (it will take 4 times as long as the above code, however). You can also download the output, "data_optim_quad.rds", [here.](https://github.com/paulstillman/prospect-estimation)
